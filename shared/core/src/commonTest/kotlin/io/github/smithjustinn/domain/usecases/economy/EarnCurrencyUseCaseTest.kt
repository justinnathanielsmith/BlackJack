...
class EarnCurrencyUseCaseTest {
    ...
    @Test
    fun `execute should add currency when amount is positive`() =
        runTest {
            val amount = 100L
    ...
    @Test
    fun `execute should not add currency when amount is zero`() =
        runTest {
            val amount = 0L
            useCase.execute(amount)
    ...
    @Test
    fun `execute should not add currency when amount is negative`() =
        runTest {
            val amount = -50L
            useCase.execute(amount)
            verifySuspend(VerifyMode.exactly(0)) { repository.addCurrency(amount) }
