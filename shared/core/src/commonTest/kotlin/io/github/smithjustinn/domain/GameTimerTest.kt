...
class GameTimerTest : BaseLogicTest() {
    ...
    @Test
    fun `timer ticks repeatedly`() = runTest {
        var tickCount = 0
        val timer = GameTimer(
            scope = testScope,
    ...
    @Test
    fun `timer stops correctly`() = runTest {
        var tickCount = 0
        val timer = GameTimer(
            scope = testScope,
            dispatchers = testDispatchers,
    ...
    @Test
    fun `restarting timer resets and doesn't duplicate ticks`() = runTest {
        var tickCount = 0
        val timer = GameTimer(
            scope = testScope,
            dispatchers = testDispatchers,
    ...
    @Test
    fun `stop before first tick prevents execution`() = runTest {
        var tickCount = 0
        val timer = GameTimer(
            scope = testScope,
            dispatchers = testDispatchers,
