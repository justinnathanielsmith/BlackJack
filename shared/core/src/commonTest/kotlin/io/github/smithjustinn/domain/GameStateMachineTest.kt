...
class GameStateMachineTest : BaseLogicTest() {
    ...
    @Test
    fun `initialization for new game with error schedules ProcessMismatch`() = runTest {
        val initialState = MemoryGameState(
        ...
        val card = stateMachine.state.value.cards.first()
        assertFalse(card.isError)
        stateMachine.stopTimer()
    }

    @Test
    fun `ScanCards action flips unmatched face-down cards up and then back down after duration`() = runTest {
        val initialState = MemoryGameState(
            cards = persistentListOf(
    ...
    @Test
    fun `ScanCards action cancels existing scan when dispatched again`() = runTest {
        val initialState = MemoryGameState(
            cards = persistentListOf(
                CardState(id = 0, suit = Suit.Hearts, rank = Rank.Ace, isFaceUp = false, isMatched = false),
            ),
        ...
        assertTrue(stateMachine.state.value.cards[0].isFaceUp)
        ...
        assertFalse(stateMachine.state.value.cards[0].isFaceUp)
        stateMachine.stopTimer()
    }

    @Test
    fun `ScanCards does not affect already matched cards`() = runTest {
        val initialState = MemoryGameState(
            cards = persistentListOf(
        ...
        assertTrue(stateMachine.state.value.cards[0].isFaceUp)
        assertTrue(stateMachine.state.value.cards[0].isMatched)
