...
class GameStateMachineTest : BaseLogicTest() {
    ...
    fun `ProcessMismatch in TimeAttack applies penalty and updates time correctly`() = runTest {
        val config = ScoringConfig(timeAttackMismatchPenalty = 10L)
        val state = GameFactory.createInitialState(pairCount = 4).copy(
        ...
        val job = testScope.launch(UnconfinedTestDispatcher(testScheduler)) {
    ...
    @Test
    fun `ProcessMismatch in TimeAttack triggers GameOver when time reaches zero`() = runTest {
        val config = ScoringConfig(timeAttackMismatchPenalty = 10L)
        val state = GameFactory.createInitialState(pairCount = 4).copy(
            mode = GameMode.TIME_ATTACK,
            config = config,
    ...
    @Test
    fun `ProcessMismatch resets unmatched cards`() = runTest {
        val config = ScoringConfig(timeAttackMismatchPenalty = 10L)
        val state = GameFactory.createInitialState(pairCount = 4).copy(
            mode = GameMode.TIME_ATTACK,
            config = config
