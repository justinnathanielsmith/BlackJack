...
class DefaultRootComponent(
    ...
    private fun createChild(config: Config, componentContext: ComponentContext): RootComponent.Child =
        when (config) {
            is Config.Start -> RootComponent.Child.Start(
                DefaultStartComponent(
                    componentContext = componentContext,
                    appGraph = appGraph,
                    onNavigateToGame = @OptIn(com.arkivanov.decompose.DelicateDecomposeApi::class) { pairs, mode, forceNewGame ->
                        navigation.push(Config.Game(pairs, mode, forceNewGame))
                    },
                    onNavigateToSettings = @OptIn(com.arkivanov.decompose.DelicateDecomposeApi::class) {
                        navigation.push(Config.Settings)
                    },
                    onNavigateToStats = @OptIn(com.arkivanov.decompose.DelicateDecomposeApi::class) {
            ...
            is Config.Stats -> RootComponent.Child.Stats(
